...
class Util
{
	...
	static function cssComp() {
		return $_SERVER["DOCUMENT_ROOT"] . "/components/link.php";
	}

	static function fragment($fragment)
	{
		return $_SERVER["DOCUMENT_ROOT"] . "/components/" . $fragment . ".php";
	}
	...
	static function mycomponent($component)
	{
		return $_SERVER["DOCUMENT_ROOT"] . "/mycomponents/" . $component . ".php";
	}
	...
	{
		$path = $_SERVER["DOCUMENT_ROOT"] . "/strings/texts/" . str_replace("-", "_", strtolower($lang)) . ".php";
		if (file_exists($path))
			return $path;
		else return $_SERVER["DOCUMENT_ROOT"] . "/strings/texts/pt-BR.php";
	}
	...
	{
		require_once $_SERVER["DOCUMENT_ROOT"] . "/system_config.php";
		if (DEBUG == true) {
			var_dump($var);
		}
	}
	...
	*/
	 ...
	 */
	static function meses(string $dataInicio, string $dataFim): int
	{
		...
		/**
		 * A diferença dos mesmes é somanda com a diferença de anos 
		 * multiplacado por 12
		 */
		$meses = ((int)$date2->format('m') - (int)$date1->format('m')
		) + (((int)$date2->format('y') - (int)$date1->format('y')
		) * 12);
