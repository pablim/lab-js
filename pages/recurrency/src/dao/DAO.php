...
class DAO {
  ...
  function __construct($class) {
    ...
    $this->tableName = Util::convertCase(
  ...
  function getQuery($queryFileName, $dir = "querys") {
    $d = __DIR__ . "/../$dir/$queryFileName";
  ...
  function save($object, $parent = null, $canCommit = true) {
    $columnsName = "";
    $columnsBinds = "";
    $binds = [];
    ...
    $classParts = explode("\\", $class);
    $table = $classParts[sizeof($classParts)-1];
    $table = Util::convertCase( $table, "pascal", "snake" );

    // associação dos objetos
    if ($parent) {
      $parentClassName = get_class($parent);
      $parentClassNameParts = explode("\\", $parentClassName);
      $reference = $parentClassNameParts[sizeof($parentClassNameParts)-1];
      $reference = Util::convertCase( $reference, "pascal", "snake" );
      $object->$reference = $parent->codigo;
    }

    try {
      //if ($parent) {
        $properties = $class::properties();
        $keyProperty = $class::getKey();
      // } else {
      //   $properties = $this->className::properties();
      //   $keyProperty = $this->className::getKey();
      // }
      ...
      foreach ($object as $property => $value) {
        if (isset($properties[$property]["unmapped"]) && 
          $properties[$property]["unmapped"]) continue;

        $camelProperty = $property;
        if (
          !Util::verify("key", $properties[$property], false) &&
          Util::verify("column", $properties[$property], true)
        ) {

          if ((isset($properties[$property]["isAssociation"]) && 
                $properties[$property]["isAssociation"])) {

							// if (is_object($value) || is_array($value)) {
							// 	if ($value["codigo"]) $object->$property = $value["codigo"];
							// 	else {
            	// 		$dependencies[] = $value;
							// 	}
							// }
            ...
            array_push(
              ...
              array(
                $property, 
                $value, 
                $this->bindTypes[$properties[$camelProperty]["type"]]
              )
            );
          }
      ...
      $sql = "INSERT INTO " . $table . "(" . $columnsName . ") "
        . " VALUES (" . $columnsBinds . ")";
      ...
      foreach ($binds as &$bind) {
        ...
        if ($bind[0] != $keyProperty) {
          ...
          if (is_object($bind[1])) {
            $associetedClass = get_class($bind[1]);
            $associetedkeyProperty = $associetedClass::getKey();
            $value = $bind[1]->$associetedkeyProperty;
            $bindType = $associetedClass::properties()[$associetedkeyProperty]["type"];
            $bindType = $this->bindTypes[$bindType];
          }

          $stmt->bindParam($bind[0], $value, $bindType);
        }
      }
      ...
      if (is_array($object))
        $object["codigo"] = $result;
      else { $object->codigo = $result; }

      // recursiva
      if ($dependencies) {
        foreach ($dependencies as $dependency) {
          if (is_array($dependency)) {
            foreach ($dependency as $dependencyObject) {
              $this->save($dependencyObject, $object, false); 
            }
          }
        }
      }
  
      if ($canCommit) $this->connection->commit();
      return $object;
    } catch (\PDOException $e) {
      throw $e;
    } catch (\Exception $e) {
  ...
  function bindProperties($object, $keyProperty) {
    ...
    foreach ($object as $property => $value) {
      ...
      array_push($binds, array($property, $value, $this->bindTypes[$properties[$camelProperty]["type"]]));
    }
